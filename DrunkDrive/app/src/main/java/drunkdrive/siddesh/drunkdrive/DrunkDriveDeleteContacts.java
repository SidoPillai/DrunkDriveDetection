...
public class DrunkDriveDeleteContacts extends Activity implements OnClickListener {
    Button b1;
    ListView lstvw;
    CheckBox cbox;
    ...
    public void onCreate(Bundle ic) {
        ...
        Cursor c = db1.query(DrunkDriveDatabase.TABLE_NAME, new String[]{DrunkDriveDatabase.COLUMN_NAME, DrunkDriveDatabase.COLUMN_NUMBER}, null, null, null, null, null);
        int i = 0;
        while (c.moveToNext()) {
        ...
        try {
            ...
            for (int i = 0; i < len; i++) {
                if (checked.get(i)) {
                    ...
                    db.delete(DrunkDriveDatabase.TABLE_NAME, "name=? and number=?", new String[]{name, num});
